 ...
 * а так же переключатель темы. Он использует контексты для управления 
 * состоянием загрузки темы.
 * 
 * @component
    ...
    const onClickHandler = (link, path) => {
        if (link !== activeLink) {
            ...
            router.push(path).then(() => {
                setLoading(false);// Откючаем состояние загрузки после перехода
            });
            setActiveLink(link)
        }
    };
    ...
    return (
        <header className={`shadow ${isDarkMode ? 'bg-gray-800' : 'bg-white'} h-16 flex items-center`} >
            <div className="container flex justify-between">
                ...
                <nav className="flex item-center gap-5">
                    {loading ? (
                        ...
                        navItems.slice(0, 5).map((item) => (
                            <a
                                onClick={() => onClickHandler(item.name, item.path)}//Передаём путь
                ...
                {/*Правая часть с полследними двумя пунктами меню */}
                ...
                <nav className="flex items-center gap-5">
                    ...
                    <fieldset className="relative inline-block w-16 h-8">
                        ...
                        <label
                            htmlFor="them-toggle"
                            className={`absolute cursor-pointer top-0 right-0 bottom-0 
